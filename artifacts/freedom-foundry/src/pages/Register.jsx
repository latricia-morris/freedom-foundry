...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const user = await apiClient.auth.register(email, password, firstName, lastName);
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      // Save optional profile fields
      try {
        await apiClient.auth.updateMe({ first_name: firstName, last_name: lastName });
        await apiClient.entities.UserProfile.create({
  ...
  const handleGoogle = () => {
    apiClient.auth.loginWithProvider("google", safeReturnTo());
  };
