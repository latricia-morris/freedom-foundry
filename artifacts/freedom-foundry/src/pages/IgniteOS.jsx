    ...
    <div className="max-w-2xl mx-auto py-16 animate-fade-in">
      <div className="mb-8">
        <div className="w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-6" style={{ background: 'linear-gradient(131deg, rgba(179,35,44,0.12), rgba(217,98,44,0.08))' }}>
          <Flame className="w-6 h-6 text-primary" strokeWidth={1.5} />
        </div>
        <h2 className="font-heading text-2xl font-light text-foreground mb-4 text-center">Ignite OS</h2>
        <div className="bg-card text-card-foreground border border-border rounded-2xl p-6 md:p-8 shadow-sm space-y-4">
      ...
      <div className="text-center">
        ...
        <h3 className="font-heading text-xl font-light text-foreground mb-2">Client Feature</h3>
...
export default function IgniteOS() {
  ...
  useEffect(() => {
    ...
    apiClient.entities.IgniteOS.filter({}, '-created_date', 1)
      .then(r => {
        const g = r?.[0] || null;
        setRecord(g);
        if (g) setForm({ ...init, ...g });
  ...
  const handleSave = async () => {
    ...
    try {
      if (record?.id) {
        await apiClient.entities.IgniteOS.update(record.id, form);
      } else {
        const created = await apiClient.entities.IgniteOS.create(form);
        setRecord(created);
      }
      setSaved(true);
  ...
  const textareaClass = "w-full rounded-xl px-4 py-3 text-sm text-foreground bg-background border border-border placeholder:text-muted-foreground outline-none focus:border-primary transition-colors resize-none";
  ...
  return (
    <div className="max-w-3xl animate-fade-in">
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground">Your brand activation operating system — from development to deployment.</p>
      </div>

      <div className="bg-card text-card-foreground border border-border rounded-2xl p-6 md:p-8 shadow-sm space-y-6">
        <div>
          <label className="block text-xs uppercase tracking-wider text-foreground/50 mb-1.5">Current Phase</label>
        ...
        <div>
          <label className="block text-xs uppercase tracking-wider text-foreground/50 mb-1.5">Activation Goals</label>
        ...
        <div>
          <label className="block text-xs uppercase tracking-wider text-foreground/50 mb-1.5">Brand Strategy</label>
        ...
        {/* Action Items */}
        <div>
          <label className="block text-xs uppercase tracking-wider text-foreground/50 mb-3">Action Items</label>
          <div className="space-y-2">
            {form.action_items.map((item, i) => (
              <div key={i} className="flex items-center gap-2">
                <button
                  onClick={() => updateAction(i, 'done', !item.done)}
                  className={`w-5 h-5 rounded border flex-shrink-0 flex items-center justify-center transition-colors ${item.done ? 'border-primary bg-primary' : 'border-border bg-background'}`}
                >
        ...
        <div>
          <label className="block text-xs uppercase tracking-wider text-foreground/50 mb-1.5">Notes</label>
