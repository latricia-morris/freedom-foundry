// Brand persona quiz definition and scoring — ported verbatim from the
// Replit-era server (lib/db/src/quiz-definition.ts) so results stay identical.
...
export function scoreQuizAnswers(answerIndexes) {
  if (answerIndexes.length !== questions.length) {
    throw new Error(`Expected ${questions.length} answers`);
  }

  const scores = Object.fromEntries(ARCHETYPE_PRIORITY.map((archetype) => [archetype, 0]));
  const coreScores = Object.fromEntries(ARCHETYPE_PRIORITY.map((archetype) => [archetype, 0]));
  ...
  questions.forEach((question, questionIndex) => {
    ...
    Object.entries(answer.scores).forEach(([archetype, points]) => {
      scores[archetype] += points ?? 0;
      if (questionIndex < 7) {
        coreScores[archetype] += points ?? 0;
      }
    });
  });
