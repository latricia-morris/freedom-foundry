...
export default function BrandChecklist() {
  ...
  const loadTasks = async () => {
    try {
      const all = await base44.entities.ChecklistTask.filter({}, '-created_date', 200);
  ...
  const handleAdd = async () => {
    ...
    try {
      await base44.entities.ChecklistTask.create({
        title: newTitle.trim(),
  ...
  const toggleComplete = async (task) => {
    ...
    try {
      await base44.entities.ChecklistTask.update(task.id, { status: newStatus });
      loadTasks();
    } catch (_) {}
  };
  ...
  const saveEdit = async (id) => {
    try {
      await base44.entities.ChecklistTask.update(id, { title: editTitle, deadline_date: editDeadline || null });
      setEditingId(null);
      loadTasks();
    } catch (_) {}
  };
  ...
  const handleDelete = async (id) => {
    try {
      ...
      await Promise.all(subs.map(s => base44.entities.ChecklistTask.delete(s.id)));
      await base44.entities.ChecklistTask.delete(id);
      loadTasks();
    } catch (_) {}
  };
  ...
  const updateSubTaskAssignee = async (id, assignee) => {
    try {
      await base44.entities.ChecklistTask.update(id, { assignee });
      loadTasks();
    } catch (_) {}
  };
