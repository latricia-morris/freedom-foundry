      ...
      try {
        ...
        if (u) {
          const [profiles, bigPic, personal, corporate, media, wbDefs, wbResponses, checklistTasks, brandUp, svcReqs] = await Promise.all([
            base44.entities.UserProfile.filter({ user_id: u.id }, '-created_date', 1).catch(() => []),
            ...
            base44.entities.WorkbookDefinition.filter({}, '-created_date', 50).catch(() => []),
            base44.entities.WorkbookResponse.filter({}, '-created_date', 200).catch(() => []),
            base44.entities.ChecklistTask.filter({}, '-created_date', 200).catch(() => []),
            base44.entities.BrandUpEntry.filter({}, '-created_date', 200).catch(() => []),
            base44.entities.ServiceRequestSubmission.filter({}, '-created_date', 50).catch(() => []),
          ]);
