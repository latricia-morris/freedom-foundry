...
export default function CorporateBrandProfile() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (profile?.id) {
        await apiClient.entities.CorporateBrandProfile.update(profile.id, form);
      } else {
        const created = await apiClient.entities.CorporateBrandProfile.create(form);
        setProfile(created);
      }
      setSaved(true);
    ...
    const timer = setTimeout(async () => {
      try {
        if (profileRef.current?.id) {
          await apiClient.entities.CorporateBrandProfile.update(profileRef.current.id, form);
        } else {
          const created = await apiClient.entities.CorporateBrandProfile.create(form);
          setProfile(created);
        }
  ...
  return (
    <div className="brand-portal-page -mx-6 lg:-mx-10 -my-6 lg:-my-10 px-6 lg:px-10 py-6 lg:py-10 min-h-[calc(100vh-160px)]">
    <div className="max-w-3xl animate-fade-in">
      <div className="mb-6 flex items-start justify-between">
        <div>
          <span className="text-[10px] uppercase tracking-[0.3em] text-muted-foreground">Brand Portal</span>
          <h1 className="font-heading text-3xl font-light text-foreground mt-1 mb-1">Corporate <span className="molten-text italic">Brand</span></h1>
          <p className="text-sm text-muted-foreground">Company identity, colors, typography, and brand strategy.</p>
        </div>
        <div className="flex items-center gap-3 mt-1">
          <button onClick={handleShare} disabled={generatingShare} className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-border text-sm transition-colors disabled:opacity-40 whitespace-nowrap text-muted-foreground hover:text-foreground hover:border-foreground/50">
      ...
      {shareLink && (
        <div className="mb-6 p-4 rounded-xl border border-border bg-card flex items-center gap-3">
          <span className="text-sm text-muted-foreground flex-1 truncate">{shareLink}</span>
          <button onClick={() => { navigator.clipboard.writeText(shareLink); toast({ title: 'Link copied', description: 'Your brand kit link is ready to share.' }); }} className="text-xs font-semibold text-muted-foreground hover:text-foreground transition-colors">Copy link</button>
        </div>
      )}
      ...
      <div className="bg-card text-card-foreground border border-border rounded-2xl p-6 md:p-8 shadow-sm space-y-8">
        ...
        <section>
          ...
          <div className="space-y-4">
            <div><label className="block text-xs uppercase tracking-wider text-muted-foreground mb-1.5">Company Name</label><input className={inputClass} value={form.company_name} onChange={e => update('company_name', e.target.value)} /></div>
            <div><label className="block text-xs uppercase tracking-wider text-muted-foreground mb-1.5">Tagline</label><input className={inputClass} value={form.tagline} onChange={e => update('tagline', e.target.value)} /></div>
            <div><label className="block text-xs uppercase tracking-wider text-muted-foreground mb-1.5">Mission Statement</label><textarea className={textareaClass} rows={3} value={form.mission_statement} onChange={e => update('mission_statement', e.target.value)} /></div>
            <div><label className="block text-xs uppercase tracking-wider text-muted-foreground mb-1.5">Target Audience</label><textarea className={textareaClass} rows={3} value={form.target_audience} onChange={e => update('target_audience', e.target.value)} placeholder="Who does this brand serve?" /></div>
          </div>
        </section>
        ...
        <section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div><label className="block text-xs uppercase tracking-wider text-muted-foreground mb-1.5">Heading Font</label><input className={inputClass} value={form.heading_font} onChange={e => update('heading_font', e.target.value)} placeholder="e.g. Cormorant Garamond" /></div>
            <div><label className="block text-xs uppercase tracking-wider text-muted-foreground mb-1.5">Subheading Font</label><input className={inputClass} value={form.subheading_font} onChange={e => update('subheading_font', e.target.value)} placeholder="e.g. Playfair Display" /></div>
            <div><label className="block text-xs uppercase tracking-wider text-muted-foreground mb-1.5">Body Font</label><input className={inputClass} value={form.body_font} onChange={e => update('body_font', e.target.value)} placeholder="e.g. Urbanist" /></div>
            <div><label className="block text-xs uppercase tracking-wider text-muted-foreground mb-1.5">Accent Font</label><input className={inputClass} value={form.accent_font} onChange={e => update('accent_font', e.target.value)} placeholder="e.g. Sacramento" /></div>
          </div>
        </section>
          ...
          <div className="space-y-2">
            {form.colors.map((color, i) => (
              <div key={i} className="flex gap-2 items-center">
                <input type="color" value={color.hex || '#000000'} onChange={e => updateItem('colors', i, 'hex', e.target.value)} className="w-12 h-10 rounded-lg border border-border cursor-pointer" />
        ...
        <section>
          ...
          <div className="space-y-4">
            ...
            <div><label className="block text-xs uppercase tracking-wider text-muted-foreground mb-1.5">Brand Personality</label><textarea className={textareaClass} rows={3} value={form.brand_personality} onChange={e => update('brand_personality', e.target.value)} placeholder="Key personality traits..." /></div>
            <div><label className="block text-xs uppercase tracking-wider text-muted-foreground mb-1.5">Positioning</label><textarea className={textareaClass} rows={3} value={form.positioning} onChange={e => update('positioning', e.target.value)} placeholder="Market position and differentiation..." /></div>
            <div><label className="block text-xs uppercase tracking-wider text-muted-foreground mb-1.5">Brand Prompts / AI Context</label><textarea className={textareaClass} rows={4} value={form.brand_prompts} onChange={e => update('brand_prompts', e.target.value)} placeholder="Key phrases and context for AI tools..." /></div>
            <div><label className="block text-xs uppercase tracking-wider text-muted-foreground mb-1.5">Brand Specs</label><textarea className={textareaClass} rows={4} value={form.brand_specs} onChange={e => update('brand_specs', e.target.value)} placeholder="Technical brand specifications for designers and marketing teams..." /></div>
          </div>
        </section>
        ...
        <section>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <label className="w-24 h-24 rounded-xl border border-dashed border-border flex items-center justify-center cursor-pointer hover:border-primary transition-colors">
              <Upload className="w-4 h-4 opacity-40" />
