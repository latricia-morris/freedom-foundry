...
export default function BrandUpAdmin() {
  ...
  const loadItems = async () => {
    try {
      const all = await apiClient.entities.BrandUpPrompt.filter({}, 'order', 200);
      setItems(all || []);
    } catch (_) {}
    setLoading(false);
  };
  ...
  const handleAdd = async () => {
    ...
    setAdding(true);
    try {
      await apiClient.entities.BrandUpPrompt.create({
  ...
  const handleBulkAdd = async () => {
    ...
    try {
      const created = await apiClient.entities.BrandUpPrompt.bulkCreate(
  ...
  const handleDelete = async (id) => {
    try {
      await apiClient.entities.BrandUpPrompt.delete(id);
  ...
  const toggleActive = async (item) => {
    try {
      await apiClient.entities.BrandUpPrompt.update(item.id, { is_active: !item.is_active });
      loadItems();
    } catch (_) {}
  };
