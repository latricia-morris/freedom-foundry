...
function ClientGate() {
  return (
    <div className="max-w-lg mx-auto py-16 text-center animate-fade-in">
      ...
      >
        <Lock className="w-7 h-7 text-primary" strokeWidth={1} />
      </div>
  ...
  return (
    <div className="max-w-3xl animate-fade-in">
      <div className="mb-6">
        ...
        <h1 className="font-heading text-3xl font-light text-foreground mt-1 mb-1">Brand <span className="molten-text italic">Guidelines</span></h1>
        <p className="text-sm text-muted-foreground">Standards, usage rules, and style notes for your brand.</p>
      </div>
        ...
        <section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {[['heading_font', 'Heading Font'], ['subheading_font', 'Subheading Font'], ['body_font', 'Body Font'], ['accent_font', 'Accent Font']].map(([k, label]) => (
              <div key={k}>
                <label className="block text-xs uppercase tracking-wider text-foreground/50 mb-1.5">{label}</label>
