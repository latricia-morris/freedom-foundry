...
export default function BrandGuidelines() {
  ...
  useEffect(() => {
    ...
    base44.entities.BrandGuidelines.filter({}, '-created_date', 1)
      .then(r => {
        const g = r?.[0] || null;
        setGuidelines(g);
        if (g) setForm({ ...init, ...g });
  ...
  const handleSave = async () => {
    ...
    try {
      if (guidelines?.id) {
        await base44.entities.BrandGuidelines.update(guidelines.id, form);
      } else {
        const created = await base44.entities.BrandGuidelines.create(form);
