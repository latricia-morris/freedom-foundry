...
export default function PersonalBrandProfile() {
  ...
  useEffect(() => {
    apiClient.entities.PersonalBrandProfile.filter({}, '-created_date', 1)
      .then(p => {
        const r = p?.[0] || null;
        setProfile(r);
        if (r) setForm({ ...init, ...r });
  ...
  const handleSave = async () => {
    ...
    try {
      if (profile?.id) {
        await apiClient.entities.PersonalBrandProfile.update(profile.id, form);
      } else {
        const created = await apiClient.entities.PersonalBrandProfile.create(form);
        setProfile(created);
      }
      setSaved(true);
    ...
    const timer = setTimeout(async () => {
      try {
        if (profileRef.current?.id) {
          await apiClient.entities.PersonalBrandProfile.update(profileRef.current.id, form);
        } else {
          const created = await apiClient.entities.PersonalBrandProfile.create(form);
          setProfile(created);
        }
    ...
    <div className="max-w-3xl animate-fade-in">
      <div className="mb-6 flex items-start justify-between flex-wrap gap-3">
        <div>
          ...
          <h1 className="font-heading text-3xl font-light text-foreground mt-1 mb-1">Personal <span className="molten-text italic">Brand</span></h1>
          <p className="text-sm text-muted-foreground">Your personal identity, bio, fonts, voice, and assets.</p>
        </div>
      ...
      <div className="bg-card text-card-foreground border border-border rounded-2xl p-6 md:p-8 shadow-sm space-y-8">
        ...
        <section>
          ...
          <div className="space-y-4">
            ...
            <div><label className="block text-xs uppercase tracking-wider text-muted-foreground mb-1.5">Business Name</label><input className={inputClass} value={form.business_name} onChange={e => update('business_name', e.target.value)} placeholder="Your business or practice name" /></div>
          </div>
        </section>
        ...
        <section>
          ...
          <div className="mb-5">
            <ArchetypeSummary view="personal" />
          </div>
          <div className="space-y-4">
            <div><label className="block text-xs uppercase tracking-wider text-muted-foreground mb-1.5">Brand Voice</label><textarea className={textareaClass} rows={3} value={form.brand_voice} onChange={e => update('brand_voice', e.target.value)} placeholder="How do you sound? What's your communication style?" /></div>
            <div><label className="block text-xs uppercase tracking-wider text-muted-foreground mb-1.5">Tonality</label><textarea className={textareaClass} rows={3} value={form.brand_tonality} onChange={e => update('brand_tonality', e.target.value)} placeholder="Describe the emotional register and energy of your brand..." /></div>
            <div><label className="block text-xs uppercase tracking-wider text-muted-foreground mb-1.5">Positioning</label><textarea className={textareaClass} rows={3} value={form.positioning} onChange={e => update('positioning', e.target.value)} placeholder="How are you positioned in the market? What's your unique stance?" /></div>
            <div><label className="block text-xs uppercase tracking-wider text-muted-foreground mb-1.5">Brand Prompts / AI Context</label><textarea className={textareaClass} rows={4} value={form.brand_prompts} onChange={e => update('brand_prompts', e.target.value)} placeholder="Key phrases, descriptors, and context for AI tools to accurately represent your brand..." /></div>
            <div><label className="block text-xs uppercase tracking-wider text-muted-foreground mb-1.5">Brand Specs</label><textarea className={textareaClass} rows={4} value={form.brand_specs} onChange={e => update('brand_specs', e.target.value)} placeholder="Technical or strategic brand specifications for designers and marketing teams..." /></div>
          </div>
        </section>
        ...
        <section>
          ...
          <div className="space-y-4">
            <div><label className="block text-xs uppercase tracking-wider text-muted-foreground mb-1.5">Short Bio</label><textarea className={textareaClass} rows={3} value={form.short_bio} onChange={e => update('short_bio', e.target.value)} placeholder="One or two sentence bio..." /></div>
            <div><label className="block text-xs uppercase tracking-wider text-muted-foreground mb-1.5">Long Bio</label><textarea className={textareaClass} rows={8} value={form.long_bio} onChange={e => update('long_bio', e.target.value)} placeholder="Full bio for press kits, speaking introductions, etc." /></div>
          </div>
        </section>
          ...
          <div className="space-y-2">
            {form.feature_links.map((link, i) => (
              <div key={i} className="flex gap-2">
                ...
                <button onClick={() => removeItem('feature_links', i)} className="px-2 text-muted-foreground hover:text-foreground"><X className="w-4 h-4" /></button>
              </div>
            ))}
