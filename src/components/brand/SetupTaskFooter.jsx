...
export default function SetupTaskFooter({ taskKey, form, onSave }) {
  ...
  const updateSetupStatus = async (status) => {
    ...
    try {
      await apiClient.entities.UserProfile.update(profile.id, { setup_status: newStatus });
  ...
  return (
    <div className="flex items-center gap-3 mt-6 pt-6 border-t border-black/10">
      ...
      <button
        ...
        disabled={saving}
        className="btn-forge flex items-center gap-2 px-6 py-2.5 rounded-lg text-xs uppercase tracking-widest disabled:opacity-50 ml-auto"
      >
