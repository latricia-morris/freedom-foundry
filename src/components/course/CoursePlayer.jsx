...
export default function CoursePlayer({ vaultItem }) {
  ...
  useEffect(() => {
    let mounted = true;
    apiClient.entities.CourseModule.filter({ vault_item_id: vaultItem.id, status: 'published' }, 'order')
      .then(async mods => {
        if (!mounted || !mods?.length) { setLoading(false); return; }
        setModules(mods);
        const lessonSets = await Promise.all(mods.map(m => apiClient.entities.CourseLesson.filter({ module_id: m.id, status: 'published' }, 'order')));
        if (!mounted) return;
        const map = {};
    ...
    apiClient.entities.LessonProgress.filter({}).then(p => mounted && setProgress(p || [])).catch(() => {});
  ...
  const toggleComplete = async (lid) => {
    ...
    if (existing) {
      await apiClient.entities.LessonProgress.delete(existing.id);
      setProgress(progress.filter(p => p.lesson_id !== lid));
    } else {
      const np = await apiClient.entities.LessonProgress.create({ lesson_id: lid, completed_at: new Date().toISOString() });
